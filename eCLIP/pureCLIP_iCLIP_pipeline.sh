...
if $SKIP_CALLING; then
  ...
  echo "pureCLIP crosslink sites calling: basic mode..."
  ...
  echo "pureCLIP: basic mode done."
  echo "generating CLmotifs..."
  ...
  dreme -p ${POOL_PREFIX}.PureCLIP.postive.fa \
    -norc -k 6 -m 4 -s 1000 -png -oc ${POOL_PREFIX}_DREME -e 0.1 \
  ...
  echo "CLmotifs done."
  echo "pureCLIP crosslink sites calling: CLmotifs mode..."
  pureclip -i ${MERGE_PREFIX}.bam \
    -bai ${MERGE_PREFIX}.bam.bai \
    -g ${FASTA} -iv 'chr1;chr2;chr3;' -nt ${THREAD} \
