...
# get rMATS lib directory
scriptDir = os.path.dirname(os.path.realpath(__file__))
rmatsLibDir = os.path.join(scriptDir, 'rMATS')
sys.path.append(rmatsLibDir)
# load rmatspipeline
from rmatspipeline import run_pipe

VERSION = 'v3.1.0 (modified by Keren Zhou)'
...
def doSTARMapping(args): ## do STAR mapping
    ...
    for i in range(len(fastqs)):
        if fastqs[i] != '':
            ##MDAMB231_sgNS_batch3_rep1_1.fatq.gz:MDAMB231_sgNS_batch3_rep1_2.fatq.gz,
            ##MDAMB231_sgNS_batch3_rep2_1.fatq.gz:MDAMB231_sgNS_batch3_rep2_2.fatq.gz
            sample = [pair.split(':') for pair in fastqs[i].split(',')]
            print "mapping the first sample"
            for rr, pair in enumerate(sample):
                ## get pair basename
                fqNameRow = os.path.basename(pair[0]).split('.')[0].split('_')
                if len(pair) == 2:
                    mapFolderName = '_'.join(fqNameRow[0:len(fqNameRow) - 1])
                else:
                    mapFolderName = '_'.join(fqNameRow)
                map_folder = os.path.join(args.od, mapFolderName);
                if args.skipMap is False:
                    if os.path.exists(map_folder):
                        if os.path.isdir(map_folder):
                            #os.rmdir(map_folder)
                            shutil.rmtree(map_folder, ignore_errors=True)
                        else:
                ...
                if args.skipMap is False:
                    ...
                    os.chdir(map_folder)
                    ## start alignment
                    cmd = 'STAR --chimSegmentMin 2 --outFilterMismatchNmax 3 --alignEndsType EndToEnd --runThreadN ' + str(args.nthread)
                    ...
                    shutil.rmtree('_STARgenome')
                    shutil.rmtree('_STARtmp')
                    ## delete .bg files
                    bgFileList = glob.glob(os.path.join(map_folder, '*.out.bg'))
...
def get_args():
    ...
    group1.add_argument('--b1', action='store', default='',
                        help='BAM configuration file.  (rep1_1:rep1_2,rep_1:rep2_2)', dest='b1')
    group2.add_argument('--b2', action='store', default='',
                        help='BAM configuration file.  (rep1_1:rep1_2,rep_1:rep2_2)', dest='b2')
    group1.add_argument('--s1', action='store', default='',
                        help='FASTQ configuration file. (rep1_1:rep1_2,rep_1:rep2_2)', dest='s1')
    group2.add_argument('--s2', action='store', default='',
                        help='FASTQ configuration file. (rep1_1:rep1_2,rep_1:rep2_2)', dest='s2')
    ...
    parser.add_argument('--skipMap', action='store_true',
                        default=False,
                        help='Skip mapping when --s1 and --s2 are set.', dest='skipMap')
    parser.add_argument('--zcat', action='store_true',
                        default=False,
                        help='input fastqs are gzipped.', dest='zcat')

    args = parser.parse_args()
    args.od = os.path.realpath(args.od)
...
def run_stat(istat, tstat, counttype, ase, cstat, od, tmp, stat):
    ...
    rmats_c = os.path.join(root_dir, 'rMATS', 'rMATS_C', 'rMATSexe')
    pas_out = os.path.join(root_dir, 'rMATS', 'rMATS_P', 'paste.py')
    inc_lvl = os.path.join(root_dir, 'rMATS', 'rMATS_P', 'inclusion_level.py')
    fdr_cal = os.path.join(root_dir, 'rMATS', 'rMATS_P', 'FDR.py')
    join_2f = os.path.join(root_dir, 'rMATS', 'rMATS_P', 'joinFiles.py')
