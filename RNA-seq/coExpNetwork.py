...
parser.add_argument('--cutoff', action='store', type=float,
                    default=0.01,
                    help='the cutoff used for adding to values before log2')
...
parser.add_argument('--gene', action='store', nargs='+', type=str,
                    help='based gene list used for testing the co-expression network (if set as "all", then run program for all genes)')
parser.add_argument('--geneFile', action='store', type=str,
                    help='1-column file (with header line) contain all based genes (if set, will ignore --gene)')
...
parser.add_argument('--input', action='store', type=str, required=True,
                    help='input gene expression matrix (DataFrame: [column:gene, row:sample], "na" values will be filtered before calculation)')
...
parser.add_argument('--opertype', action='store', type=str,
                    default='either',
                    choices=['either', 'all'],
                    help='apply operators on either or all columns of DataFrame')
parser.add_argument('--output', action='store', type=str, required=True,
                    help='output result matrix')
parser.add_argument('--sep', action='store', type=str,
                    default='\t',
                    help='delimiter of columns to use ($\'\\t\' for tab)')
    ...
    for i in range(len(igAllData.columns)):
        ...
        for tgene in tgeneList:
            ...
            for j in range(len(tgAllData.columns)):
                ...
                qAve = np.average(nparrA)
                tAve = np.average(nparrB)
                ## to avoid 0 elements
                if len(nparrA) == 0:
                    continue
                ...
                if tsampleSize >= minSize:
                    if args.log2 is True:
                        nparrA = np.log2(nparrA + args.cutoff)
                        nparrB = np.log2(nparrB + args.cutoff)
                    if method == 'pearson':
                ...
                if rho != 0:
                    if tgene not in coefDict:
                        coefDict[tgene] = [qAve, tAve, rho, pvalue, sampleSize]
                    else:
...
# check input gene or gene file
if bool(args.gene) is False and bool(args.geneFile) is False:
    sys.stderr.write('--gene or --geneFile should be set!\n')
    sys.exit()

if bool(args.geneFile):
    inputGeneList = []
    with open(args.geneFile, 'r') as f:
        __ = f.readline()
        for line in f:
            row = line.strip().split('\t')
            inputGeneList.append(row[0])
    args.gene = sorted(set(inputGeneList))
...
with open(args.output, 'w') as out:
    row = ['inputGene', 'testGene', 'inputAveExp', 'testAveExp', 'pcc', 'pvalue', 'sampleSize']
    out.write('\t'.join(row) + '\n')
