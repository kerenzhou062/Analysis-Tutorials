...
def SpanSmooth(valueList, method, span):
    def span_ave(vlist, rstart, rend, rlen):
        return sum(vlist[rstart:rend]) / rlen
    ## sub main
    ...
    if method == 'none':
        ...
        for i in range(vlength):
            if i < span:
                ...
                rstart, rend, rlen= [i-span + 1, i + 1, span]
...
def Smooth(valueList, method, span, count=1):
    smoothList = valueList
    for i in range(count):
        smoothList = SpanSmooth(smoothList, method, span)
        smoothList = SpanSmooth(smoothList[::-1], method, span)
...
def RebuildBed(bedFile, method, extend):
    ...
    with open(bedFile, 'r') as f:
        for line in f:
            ...
            buildbed.score = 1
            uniqPeakName = '|'.join([buildbed.name, str(lineNum)])
            if method == 'center':
                ## get the center of the exon
                if colNum >= 12:
                    buildbed = buildbed.decode()
                    exonList = buildbed.exon
                    halfLength = int(buildbed.exonlength / 2)
                    exonLenSum = 0
                    for exon in exonList:
                        exonLength = exon[1] - exon[0]
                        if exonLength + exonLenSum >= halfLength:
                            center = halfLength - exonLenSum + exon[0]
                            break
                        else:
                            exonLenSum += exonLength
                else:
                    center = int((buildbed.end + buildbed.start) / 2)
                row[1] = str(center - extend)
                row[2] = str(center + extend + 1)
                lineRow = row[0:3]
                name = '##'.join([uniqPeakName, '1'])
                lineRow.extend([name, str(buildbed.score), buildbed.strand])
                bedLineRow.append('\t'.join(lineRow))
            elif method == 'exon':
                if colNum >= 12:
                    bed12 = buildbed.decode()
                    for i in range(len(bed12.exon)):
                        exonId = str(i + 1)
                        name = '##'.join([uniqPeakName, exonId])
                        lineRow = [bed12.chr, bed12.exon[i][0], bed12.exon[i][1]]
                        lineRow.extend([name, buildbed.score, buildbed.strand])
                        bedLineRow.append('\t'.join(map(str,lineRow)))
                else:
                    sys.stderr.write('input should be in bed12 format!')
            else:
                if extend > 0:
                    center = int((buildbed.end + buildbed.start) / 2)
                    row[1] = center - extend
                    if row[1] < buildbed.start:
                        row[1] = buildbed.start
                    elif row[1] < 0:
                        row[1] = 0
                    else:
                        row[1] = center - extend
                    row[2] = center + extend + 1
                    if row[2] > buildbed.end:
                        row[2] = buildbed.end
                    row[1] = str(row[1])
                    row[2] = str(row[2])
                lineRow = row[0:3]
...
def AnnoBed12ToBed6(bed12File, geneType, feature, binType, binsize):
    ...
    with open(bed12File, 'r') as f:
        for line in f:
            ...
            buildbed = bedutils.buildbed(row)
            bed12 = buildbed.decode()
            bed12Dict[buildbed.name]['info'] = buildbed
            bed12Dict[buildbed.name]['full'] = [[buildbed.start, buildbed.end]]
            bed12Dict[buildbed.name]['exon'] = bed12.exon
            bed12Dict[buildbed.name]['intron'] = bed12.intron
            ## length and size
            bed12Dict[buildbed.name]['full_size'] = bed12.end - bed12.start
            bed12Dict[buildbed.name]['intron_size'] = ExonLenSum(bed12.intron)
            bed12Dict[buildbed.name]['exon_size'] = ExonLenSum(bed12.exon)

            if geneType == 'protein_coding':
                if len(bed12.cds) == 0:
                    continue
                bed12Dict[buildbed.name]['utr5'] = bed12.utr5
                bed12Dict[buildbed.name]['cds'] = bed12.cds
                bed12Dict[buildbed.name]['utr3'] = bed12.utr3
            else:
                if len(bed12.cds) > 0:
                    continue
            ...
            if binType == 'average' and feature == 'coding':
                for key in ['utr5', 'cds', 'utr3']:
                    statsDict[key] += ExonLenSum(bed12Dict[buildbed.name][key])
            count += 1
    ...
    for name in sorted(bed12Dict.keys()):
        ...
        for feature in sorted(binsizeDict.keys()):
            ...
            for coord in tempDict[feature]:
                ...
                row = [buildbed.chr, coord[0], coord[1], coordName, mapLenPerbp, buildbed.strand]
...
def RunMetagene(inputBedDict, args, kwargs):
    ...
    ## smooth values continually
    smmothBinValList = Smooth(binValList, args.smooth, args.width)
    for count,feature in enumerate(binFeatureList):
        binVal = smmothBinValList[count]
        if args.type != 'none':
            binVal = round(binVal, 5)
        finalBinValDict[count + 1]['feature'] = feature
        finalBinValDict[count + 1]['value'] = binVal
