...
function showHelp {
  ...
  echo -e "options:
    ...
    --cits: # bp that cits closed to A <float>
    ...
    --qval: qvalue cutoff <float>
    --pval: pvalue cutoff <float>
...
TEMP=`getopt -o hb:e:g:i:o:p:t:, --long help,skip-mapping,skip-calling, \
  ...
  --long cits:,input:,thread:,output:,pval:,qval:,exp-prefix:,pool-prefix:,index:, \
  --long max-mismatch:,full-bed:,gtf:,repeat-bed:,barcode-length:,fasta:, \
  -- "$@"`
...
echo "FASTA=$FASTA"
echo "GTF=$GTF"
echo "CITS_DIS=$CITS_DIS"
echo "PVAL_CUTOFF=$PVAL_CUTOFF"
echo "QVAL_CUTOFF=$QVAL_CUTOFF"
  ...
  if $STAR_FLAG; then
    ...
    if [ -z $GENOME_INDEX ]; then
      ...
      if [[ ! -f $GENOME_INDEX/Genome ]]; then
        ...
        READ_AVG_LENGTH=`echo "${READ_AVG_LENGTH} ${BARCODE_LEN}" |awk '{print $1-$2}'`
        echo "Reads length after removing barcodes:${READ_AVG_LENGTH}"
        cd $GENOME_INDEX
        ### build index
    ...
    ## Collapse exact duplicates
    {
      ...
      if $STAR_FLAG; then
        ...
        STAR --outSAMtype BAM SortedByCoordinate --runThreadN ${MAP_THREAD} \
          ...
          --outSAMreadID Standard --outReadsUnmapped None \
...
if $SKIP_CALLING; then
  ...
  rbsSeeker -T CT -L 20 -t 129600000 -n 1 -H 3 -d 1 \
    -p $PVAL_CUTOFF -q $QVAL_CUTOFF -o "$RESULT_DIR" -P "$POOL_PREFIX" \
...
ln -sf ${RESULT_DIR}/${POOL_PREFIX}_CT.bed ./${POOL_PREFIX}_rbsSeeker_CIMS.bed
ln -sf ${RESULT_DIR}/${POOL_PREFIX}_Truncation.bed ./${POOL_PREFIX}_rbsSeeker_CITS.bed

echo "Pooling CT and Truncation m6A sites..."

## for CIMS, required RRACH occur at index 7 
for i in `find ./ -type l -name "${POOL_PREFIX}*.bed" | grep -E "rbsSeeker_CIMS"`;
do
...
## for CITS, required RRACH occur at index 7 or before CITS_DIS
for i in `find ./ -type l -name "${POOL_PREFIX}*.bed" | grep -E "rbsSeeker_CITS"`;
do
  outputName="${i//_rbsSeeker_/.rbsSeeker.}"
  awk -v cits="$CITS_DIS" 'BEGIN{OFS="\t";FS="\t";}
  {
    if(FNR>1){
      if($8 <= 7 && $8 >= (7 - cits + 1) && $8 >= 0){
        seq = substr($7, $8 + cits, 5);
        if ($6 == "+") {
          $2 = $2 - cits;
          $3 = $2 + 1;
        }else{
